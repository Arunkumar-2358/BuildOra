...
export const LandingPage = () => (
  <main className="text-slate-100">
    <section className="relative overflow-hidden border-b border-slate-800/60 bg-mesh-radial">
      <div className="mx-auto grid min-h-[calc(100vh-76px)] max-w-7xl items-center gap-10 px-4 py-12 lg:grid-cols-[1.02fr_0.98fr]">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative z-10">
          <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-4 py-2 text-sm font-bold text-blue-100 shadow-sm backdrop-blur-xl">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10">
          ...
          <div className="absolute -bottom-6 left-4 right-4 rounded-2xl border border-slate-700/40 bg-slate-900/90 p-4 shadow-crisp backdrop-blur-xl md:left-8 md:right-auto md:w-[25rem]">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Live lead board</p>
                <h3 className="mt-1 text-lg font-extrabold text-slate-100">High-intent requirements</h3>
              </div>
            ...
            <div className="mt-4 space-y-3">
              {activeLeads.map(([title, city, budget, bids]) => (
                <div key={title} className="flex items-center justify-between gap-3 rounded-xl bg-slate-800/80 p-3">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-extrabold text-slate-100">{budget}</p>
                    <p className="text-xs font-semibold text-blue-300">{bids}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {[
            ...
            <div key={title} className="premium-card rounded-2xl p-6 transition hover:-translate-y-1 hover:shadow-glow">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-extrabold text-slate-500">0{index + 1}</span>
              </div>
    ...
    <section className="py-20">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          <p className="font-bold text-blue-300">Demo-ready workflow</p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {["Role-based dashboards", "Bid status controls", "Realtime chat", "Contractor portfolio profiles"].map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-xl border border-slate-700 bg-slate-900 p-3 text-sm font-bold text-slate-200">
        ...
        <div className="rounded-2xl bg-slate-900 p-4 shadow-soft">
          <div className="rounded-2xl bg-slate-950 p-4">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700/50 pb-4">
              <div>
                <p className="text-xs font-bold uppercase text-slate-400">Project pipeline</p>
                <h3 className="text-xl font-extrabold text-slate-100">Premium interiors, Jubilee Hills</h3>
              </div>
              <span className="rounded-full bg-blue-500/20 px-3 py-1 text-xs font-bold text-blue-300">Award pending</span>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {["Urban Nest Studio", "Craft Axis", "StoneLine Builds"].map((name, index) => (
                <div key={name} className="flex items-center justify-between rounded-xl border border-slate-700 p-3">
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-slate-800 text-sm font-extrabold text-white">{index + 1}</span>
                    <div>
                      <p className="font-bold text-slate-100">{name}</p>
                      <p className="text-sm text-slate-400">Detailed quote submitted</p>
                    </div>
                  </div>
                  <MoveRight className="h-5 w-5 text-blue-300" />
                </div>
              ))}
            </div>
          </div>
    ...
    <section id="contractors" className="py-20">
      <div className="mx-auto max-w-7xl px-4">
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="font-bold text-blue-300">Top contractors</p>
    ...
    <section className="py-20 text-white">
      <div className="mx-auto grid max-w-7xl gap-5 px-4 md:grid-cols-3">
        {[
          ...
          <div key={title} className="rounded-2xl border border-slate-700 bg-slate-900/60 p-6">
      ...
      <div className="mx-auto grid max-w-7xl gap-5 px-4 md:grid-cols-3">
        {[
          ...
          <blockquote key={quote} className="premium-card rounded-2xl p-6 text-lg font-semibold leading-8">
            ...
            “{quote}”
            <footer className="mt-4 text-sm font-bold text-slate-400">{author}</footer>
          </blockquote>
        ))}
      </div>
    </section>
